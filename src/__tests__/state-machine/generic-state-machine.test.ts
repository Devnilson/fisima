...
describe('deterministic-state-machine', () => {
  it('should start at inital state', () => {
    const machine = new GenericStateMachine('A', new Set(['A']), (_state, _event) => _state);
  ...
  it('should update current state when transitioning', () => {
    const machine = new GenericStateMachine('A', new Set(['A']), (_state, _event) => ({ currentNode: 'B' }));
  ...
  describe('canClose', () => {
    ...
    it('should not be false when not at final state', () => {
      const machine = new GenericStateMachine('A', new Set(['B']), (_state, _event) => ({ currentNode: 'B' }));
